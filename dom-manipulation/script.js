...
async function fetchQuotesFromServer() {
  try {
    ...
    resolveConflicts(serverQuotes);
    saveQuotes(); // Save updated quotes to local storage
    showNotification('Quotes have been synchronized with the server.');
  } catch (error) {
    console.error('Error fetching quotes from server:', error);
  }
}
...
// Function to populate categories for filtering
...
const categoryFilterHTML = `<select id="categoryFilter"><option value="">All Categories</option></select>`;
document.body.insertAdjacentHTML('beforeend', categoryFilterHTML);
